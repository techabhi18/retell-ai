...
const CallList = () => {
    ...
    const isLastPage = !hasMoreData && currentPage * callsPerPage >= allCalls.length;
                            ...
                            <table className="min-w-full text-sm text-left text-gray-700">
                                <thead className="bg-gray-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-2 whitespace-nowrap">Disconnection Reason</th>
                                        <th className="px-4 py-2 whitespace-nowrap">User Sentiment</th>
                                ...
                                <tbody>
                                    {displayedCalls.map((call, index) => (
                                        ...
                                        >
                                            <td className="px-5 py-3">{(currentPage - 1) * callsPerPage + index + 1}</td>
                                            <td className="px-5 py-3">{new Date(call.start_timestamp).toLocaleString()}</td>
                                            <td className="px-5 py-3">{formatDuration(call.call_cost?.total_duration_seconds) || "0"} sec</td>
                                            <td className="px-5 py-3">
                                                ₹{call.call_cost?.combined_cost?.toFixed(2) || "0.00"}
                                            </td>
                                            <td className="px-5 py-3">{call.call_status || "-"}</td>
                                            <td className="px-5 py-3">{call.disconnection_reason || "-"}</td>
                                            <td className="px-5 py-3">{call.call_analysis?.user_sentiment || "-"}</td>
                                            <td className="px-5 py-3">{call.call_analysis?.call_successful ? "Yes": "No" || "-"}</td>
                                            <td className="px-5 py-3">{call.from_number || "-"}</td>
                                            <td className="px-5 py-3">{call.to_number || "-"}</td>
                                            <td className="px-5 py-3 text-blue-500 underline">Call Logs</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                <div className="p-6 h-full overflow-y-auto">
                    ...
                    {selectedCall && (
                        <div className="space-y-3 text-sm">
                            <div>
                                <strong>Time:</strong> {new Date(selectedCall.start_timestamp).toLocaleString()}
                            </div>
                            <div>
                                <strong>Duration:</strong> {formatDuration(selectedCall.call_cost?.total_duration_seconds) || "0"} sec
                            </div>
                            <div>
                                <strong>Cost:</strong> ₹
                            ...
                            <div>
                                <strong>Disconnection Reason:</strong> {selectedCall.disconnection_reason}
                            </div>
                            <div>
                                <strong>User Sentiment:</strong> {selectedCall.call_analysis?.user_sentiment}
                            </div>
                            <div>
                            ...
                            <div className="bg-gray-50 p-3 rounded text-xs max-h-[400px] overflow-y-auto">
                                {selectedCall?.call_analysis?.call_summary || "Summary not available"}
                            </div>
                            <div>
